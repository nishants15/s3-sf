    ...
    stages {
        ...
        stage("Authenticate with Snowflake") {
            steps {
                ...
                sh 'sudo -E snowsql -a kx23846.ap-southeast-1 -u mark -p 12345678 -r accountadmin -w compute_wh -d dev_convertr -s stage -f create_stage.sql'
        ...
        stage('Copy data from S3 to Snowflake') {
            steps {
                // Use sudo with -E option to preserve environment variables
                sh 'sudo -E snowsql -a kx23846.ap-southeast-1 -u mark -p 12345678 -r accountadmin -w compute_wh -d dev_convertr -s stage -f copy_data.sql'
            }
        }
    }
}
